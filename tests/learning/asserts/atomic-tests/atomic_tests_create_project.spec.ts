...
test.describe("Atomic Tests: Create Project Modal", () => {
  ...
  test("Modal Structure Structure", async () => {
    ...
    await test.step("Priority Select", async () => {
      ...
      await expect.soft(newProjectModal.prioritySelect).toHaveText("*Priority");
      await expect.soft(newProjectModal.priorityLabel).toHaveValue("34");
      ...
      await newProjectModal.selectPriority("Hight");
