...
test.describe("Froms Actions", () => {
  ...
  test("pressSequentially and fill", async ({ page }) => {
    ...
    await nameInput.clear(); // Vyčistí hodnotu pole používat jen u pressSequentially jinak nečistit
    await nameInput.pressSequentially("Dlouhý text...", { delay: 200 }); // zpomalení
  });
  ...
  test("Radio, Checkbox check", async ({ page }) => {
    ...
    await page.locator("#interests-sports").uncheck(); //odkliknutí
  });

  // když chci vyplnit datum tak ve formátu uřčený ISO 6001
  ...
  // takto se to dělá vždy když chci otestovat vložení souboru
